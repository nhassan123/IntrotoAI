<!DOCTYPE html>
<!-- saved from url=(0166)https://markus.teach.cs.toronto.edu/csc384-2018-01/en/assignments/2/submissions/download?file_name=propagators.py&grouping_id=511&id=2&path=%2F&revision_identifier=15 -->
...
def prop_FC(csp, newVar=None):
    pruned = []
    if newVar != None:
        cons_newVar = csp.get_cons_with_var(newVar)
        for cons in cons_newVar:
            if(cons.get_n_unasgn()!=1):
                continue
            else:
                unassign_var = cons.get_unasgn_vars()[0]
                notDWO = FCCheck(cons, unassign_var, pruned)
                if (notDWO):
                    continue
                else:
                    return False, pruned
        ...
        for cons in all_cons:
            num_assign = cons.get_n_unasgn
            if(num_assign != 1):
                continue
            else:
                unassign_var = const.get_unasgn_vars()[0]
                notDWO = FCCheckcons(cons, unassign_var, pruned)
                if(not(notDWO)):
                    return False, pruned
    return True, pruned
...
def is_False(C,x,d):
    ...
    for var in scopeC:
        if(not(var.name == x.name)):
            assignment.append(var.get_assigned_value())

        else:

            assignment.append(d)
    satisfy = C.check(assignment)
...
def prop_GAC(csp, newVar=None):
    ...
    if(not(newVar)):
        ...
        if(not(cons_newVar)):
            return None
        for c in cons_newVar:
            GACQueue.append(c)

    DWOnotHappened = enforce_GAC(csp, GACQueue, pruned)
    if(DWOnotHappened):
        return True, pruned
...
def enforce_GAC(csp,queue,pruned):
    while(len(queue))&gt;0:
        ...
        for v in memC:
            ...
            cur_dom = v.cur_domain()
            for d in cur_dom:


                existsA = C.has_support(v,d)
                ...
                if not(existsA):
                    ...
                    if(not(v.cur_domain())):
                        ...
                        cons_v = csp.get_cons_with_var(v)
