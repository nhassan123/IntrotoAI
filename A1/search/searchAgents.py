...
class CornersProblem(search.SearchProblem):
    ...
    #Stores the walls, pacman's starting position and corners.
        ...
        self.VisCorners = [self.corners[0], self.corners[1], self.corners[2], self.corners[3]]
        self._expanded = 0
        self.i = 0
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                ...
                #self.VisCorners = cornersLeft #
                nextState = (nextPosition, tuple(cornersLeft))
                cost = 1
                successors.append( ( nextState, action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    while len(unVisitedCorners) != 0:
        dist,corner = manhattanHeuristic2(position,unVisitedCorners)
        h += dist
...
def manhattanHeuristic2(position,cornersLeft):
    "The Manhattan distance heuristic for a PositionSearchProblem"
    min_dist = None
    for i in range(len(cornersLeft)):
        dist = util.manhattanDistance(position,cornersLeft[i])
...
def foodHeuristic(state, problem):
    ...
    closeFood = manhattanHeuristic3(position,foodList)
    dist = mazeDistance(closeFood,position,gs)
    
    #You can eat dots along the same row/column as you're moving
    leftPoints = 0
    for (x,y) in foodList:
        flag = 0
        if x!=position[0] and x!=closeFood[0]:
            leftPoints = leftPoints + 1
            flag = 1
        
        if flag == 0:
            if y!=position[1] and y!=closeFood[1]:
                leftPoints = leftPoints + 1
    
    return dist + leftPoints

def manhattanHeuristic3(position,cornersLeft):
    "The Manhattan distance heuristic for a PositionSearchProblem"
    
    min_dist = util.manhattanDistance(position,cornersLeft[0])
    close = cornersLeft[0]
